...
| `deterministic_torch` | `true` | Enable fully deterministic PyTorch behaviour (`cudnn.deterministic=True`, `cudnn.benchmark=False`, `use_deterministic_algorithms(True)`). Ensures reproducible results at the cost of some performance. Set to `false` for faster training when exact reproducibility is not needed. |
